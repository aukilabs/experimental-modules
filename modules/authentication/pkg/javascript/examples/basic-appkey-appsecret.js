...
async function main() {
  ...
  const config = {
    ...
    refreshUrl:
      process.env.REFRESH_URL || "https://api.aukiverse.com/user/refresh",
  ...
  if (!credentials.appKey || !credentials.appSecret) {
    console.error("❌ APP_KEY and APP_SECRET must be set in .env file");
    process.exit(1);
  }

  console.log("Using App Key/App Secret authentication");
  ...
  console.log(
    ...
    client.isAuthenticated() ? "Authenticated" : "Not authenticated"
  );
  console.log();
  ...
  try {
    ...
    if (!domainId) {
      ...
      console.log(
        `  Server Region: ${domainAccess.domain_server.cloud_region}`
      );
      console.log(
        `  Location: ${domainAccess.domain_server.latitude}, ${domainAccess.domain_server.longitude}`
      );
      console.log(
        `  Access token: ${domainAccess.access_token.substring(0, 50)}...`
      );
